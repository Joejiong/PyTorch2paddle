...
def _contains_weights_pf(paddle_h5_layer):
    for key in _WEIGHT_KEYS:
        if key in paddle_h5_layer:
            return True
    return False


def dig_to_params_pf(paddle_h5_layer):
    while not _contains_weights(paddle_h5_layer):
        paddle_h5_layer = paddle_h5_layer[list(paddle_h5_layer.keys())[0]]

    return paddle_h5_layer
